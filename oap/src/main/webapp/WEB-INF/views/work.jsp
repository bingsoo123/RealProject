...
<script>
	...
	function init() {
		...
		addTask.addEventListener('click', function() {
			window.open("/WorkAdd", "업무 추가",
					"width=550, height=450, menubar=no, status=no, toolbar=no")
		});
		addTask.value = "업무 추가";
	...
	function eTest(num) {
		...
		window
				.open("", "pop",
						"width=550, height=450, menubar=no, left=450, top=100, status=no, toolbar=no");
		form.submit();
	...
	function taskTypeSelect(num) {
		...
		window
				.open("", "pop",
						"width=550, height=450, left=450, top=100, menubar=no, status=no, toolbar=no");
		form.submit();
	...
	function taskType(tlCommentData) {
		...
		for (i = 0; i < tlCommentData.length; i++) {
			...
			let taskContent = document.createElement('div');
			taskContent.className = "taskContent";
			taskContent.textContent = tlCommentData[i].mtDetail.replace(/\+/g,
					" ");// 추가했음 replace
			taskContent.id = "taskName" + i;
			taskContent.value = task[i].mtDetail;
	...
	function goDelete(index) {
		...
		request.setRequestHeader("Content-Type",
				"application/x-www-form-urlencoded;charset=UTF-8");
		request.send("sCode=Drop" + "&shCode=" + shCode + "&tlNumber="
				+ tlNumber + "&mtDetail=" + mtDetail);
	}
